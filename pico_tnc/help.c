...
static const char *const help_lines_en[] = {
    ...
    "",
    "=== Network ===",
    "UNPROTO (unproto jn1dff-14 v(via) jn1dff-1) - 3 digis max",
    "MONitor (mon off|me|we|all)",
    "",
    "=== Operation ===",
    "CONverse (con)",
    ...
    "DIGIpeat (digi off|alias|mycall|both; digi on = both)",
    "",
    "=== GPS / Sensor ===",
    "GPS (gps on|off, gps baud [auto|rate], gps diag, gps $GPGGA/$GPGLL/$GPRMC)",
    "",
    "=== Signature / QSL ===",
    "PRIVKEY (privkey show, privkey gen [m|p|mona1|p2pkh|p2sh|p2wpkh], privkey set [m|p|mona1|p2pkh|p2sh|p2wpkh|WIF|RAW])",
    "SIGN (sign msg <text>, sign adv -name ... -bio ..., sign qsl ..., sign recovery {JSON}<sig>)",
    "",
    "=== Hardware ===",
    ...
    "SYSTEM (system usb_bootloader) - guarded reboot to USB BOOTSEL mode",
    "",
    "=== Diagnostics ===",
    "CALIBRATE (Calibrate Mode - Testing Only)",
    "TERMTEST (termtest) - Raw terminal byte inspector (Ctrl+C to exit)",
...
static const char *const help_lines_ja_utf8[] = {
    ...
    "",
    "=== Network ===",
    "UNPROTO (unproto jn1dff-14 v(via) jn1dff-1) # 宛先.経路設定 デジピータは最大3局",
    "MONitor (mon off|me|we|all) # 受信パケットフィルタ",
    "",
    "=== Operation ===",
    ...
    "DIGIpeat (digi off|alias|mycall|both; digi on=both) # 中継機能",
    "",
    "=== GPS / Sensor ===",
    "GPS (gps on|off, gps baud [auto|rate], gps diag, gps $GPGGA/$GPGLL/$GPRMC) # GPS/APRS設定",
    "",
    "=== Signature / QSL ===",
    ...
    "SIGN MSG     (sign msg <text>) # テキスト署名して送信待機",
    "SIGN ADV     (sign adv -name ... -bio ...) # 署名アドバタイズ送信待機",
    "SIGN QSL     (sign qsl [args]) # QSLテンプレート署名(引数またはウィザード)",
    "SIGN RECOVERY(sign recovery {JSON}<署名>) # 受信処理と同じ署名検証を手入力で再現",
    "",
    "=== Hardware ===",
    ...
    "SYSTEM (system usb_bootloader) # 10秒以内に Y/E/S/Enter を順に入力すると USB BOOTSEL へ移行",
    "",
    "=== Diagnostics ===",
    "CALIBRATE (calibrate) # 送信調整用トーン出力)",
    "TERMTEST (termtest) # 受信バイトを生表示する端末検査モード (Ctrl+Cで終了)",
