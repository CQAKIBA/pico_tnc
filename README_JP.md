...
|PRIVKEY|privkey show, privkey gen p2pkh, privkey gen m, privkey set p2wpkh, privkey set `<64hex>`, privkey set p2wpkh:`<WIF>`|Monacoin秘密鍵スロットの表示/生成/設定．`show`はセキュリティ確認後に秘密情報を表示．保存対象は32byte生鍵+compressed+active typeのみ．`set m/p/mona1/p2pkh/p2sh/p2wpkh`はactive typeのみ正規化して更新．typed WIFはactive typeを更新，通常WIF/RAWは既存active typeを維持|
